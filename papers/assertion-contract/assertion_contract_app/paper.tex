...
For our offline distributed verification method, let's assume that the parameter is valid, and now we consider the cost to verify the parameter and perform the actual work. Let $m$ be the number of proofs required to ensure the correctness of the parameter. Thus, the offline method costs the gas for submitting the parameter, the gas to store the parameter (or its hash), $m$ times the gas to submit the seed, check the condition of the parameter point associated with the seed, the gas to compute the hash and compare it with the target hash, and of course the gas to run the computation of the actual work.
...
\begin{equation}
G_{off} = G_{parameter} + G_{storage} + m \cdot (G_{seed} + G_{check} + G_{hash}) + G_{run}
\end{equation}

\begin{itemize}
    \item \( G_{parameter} \): Intrinsic gas for submitting the parameter.
    \item \( G_{storage} \): Gas for storing the parameter or its hash.
    \item \( G_{seed} + G_{check} + G_{hash} \): Gas for each of the \( m \) seeds to:
    \begin{itemize}
...
\begin{itemize}
    ...
    \item In the offline method, this cost is \( m \cdot G_{check} \), where \( m \) is the number of seeds. Typically, \( m \) is significantly smaller than \( n \), as seeds represent a subset of checks.
\end{itemize}

\paragraph*{Additional Costs in the Offline Method}
\begin{itemize}
    \item The offline method includes \( m \cdot G_{seed} \) for submitting each seed.
    \item It also includes \( m \cdot G_{hash} \) for computing and comparing hashes, which the on-chain method does not explicitly incur.
\end{itemize}
...
Let \( G_{m} \) be the sum of the gas costs to perform \( m \) times the parameter checkpoint:
\[
G_{m} = m \cdot (G_{seed} + G_{check} + G_{hash})
\]

In case the base gas does not contain the storage gas, then the \( G_m \) gas includes it.
...
\begin{figure}
  ...
  \begin{tikzpicture}
    \begin{axis}[
        ...
        yticklabels={$G_{base} + G_{check}$,$G_{base} + 2 \cdot G_{check}$, $G_{base} + 3 \cdot G_{check}$,,$G_{base}+ G_m$,,$G_{base} + n \cdot G_{check}$,},
    ]
      \addplot[
        color=red,
        mark=square,
...
% \end{figure}
Overall, the offline method (\( G_{off} \)) is generally more cost-effective for large parameters as it reduces the number of on-chain checks from \( n \) to \( m \). However, the exact cost-effectiveness depends on the relative sizes of \( n \) and \( m \), the parameter storing gas, and the specific gas costs of \( G_{seed} \) and \( G_{hash} \).
