...
as soon as an answer is available. The issue of trustworthiness of the
oracle arises because it is an entity outside of the blockchain, which
injects information into an otherwise closed, trusted (or trustless)
environment.

However, oracles are expensive and there are other usecases for
connecting a contract with traditional software. One example is
automatizing procedures like managing an auction, automatic bidding in
...
such as the Ethereum JSON-RPC API and the Tezos RPC API, but they
