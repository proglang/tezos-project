...
Building such oracles requires programmatic interfaces to implement the interactions. However, current blockchains often provide non-programmatic interfaces, such as the Ethereum JSON-RPC API and the Tezos RPC API. We present a typed API for invoking contracts from OCaml programs. Our type API supports the implementation of programs that safely interact with smart contracts on the blockchain. Such an API with a reliable computing facility and network connectivity is sufficient to self-provision an oracle. Moreover, our approach provides a type-safe facility instead of oracles, which are intrinsically untyped as they aim to provide a generic service.
...
# let auction = Cl.make_contract_from_hash auction_hash
...
$\TCONTRACT\ \TYPE\ \TYPEU$ is a public hash that is verified to be associated
with a contract with parameter type $\TYPE$ and storage $\TYPEU$. Operation hashes \OPH\
...
\begin{definition}%[State of a node]
  The \emph{state of a local node} is a pair
  $\NODE = [ \EXPRS, \ACCOUNTS
  % , \OPERATIONS
  ]$, where $\EXPRS$ is a
...
implementation provides all of these types and operations via a library module.
...
the parameter and storage types when originating a contract. Otherwise, they are set to the
uninformative type $\TNO$. We take the liberty of omitting uninformative
...
$\inferrule{}{\TPUH \SubType \TADDR}$, $\inferrule{}{\TPUK \SubType
  \TADDR}$, $\inferrule{}{\TCONTRACT\ \TYPE\ \TYPEU \SubType
  \TCONTRACT\ \TYPE\ \TNO}$, $\inferrule{}{\TCONTRACT\ \TYPE\ \TNO
  \SubType \TCONTRACT\ \TNO\ \TNO}$, and $\inferrule{}{\TCONTRACT\
  \TNO\ \TNO \SubType \TPUH}$. Similarly for operation hashes: $\inferrule{}{\TOPH\ \TYPE\ \TYPEU \SubType
  \TOPH\ \TYPE\ \TNO}$ and $\inferrule{}{\TOPH\ \TYPE\ \TNO
  \SubType \TOPH\ \TNO\ \TNO}$. 
...
We express the subtyping relation via explicit cast operations because
the host language OCaml does not support subtyping. The rationale of
the subtyping among contract types is determined by the usage
pattern. If we wish to mix uses of different contracts with the same
parameter type but different storage, then we cast them to the common
type $\TCONTRACT\ \TYPE\ \TNO$. Once we have cast a contract to type
$\TCONTRACT\ \TNO\ \TNO$, we can put it in a list with any other
contract, but the only possible operation is querying its balance
(after casting further to $\TADDR$). The function
\lstinline/make_contract_from_hash/ mentioned in
Section~\ref{sec:motivation} corresponds to a cast from $\TPUH$ to
$\TCONTRACT\ \TYPE_p\ \TYPE_s$ where $\TYPE_p$ and $\TYPE_s$ are given
by the parameters \lstinline/parameter/ and \lstinline/storage/,
respectively.  The type parameters for operation
hashes just communicate the parameters to the result of the
$\CONTRACT$ operation, so their subtyping rules are analogous to the
one for the contract type. 
...
unspecified. Figure~\ref{fig:typing-blockchain-operations} gives the
typing of contract operations and queries. A standard transfer or
contract invocation results in an uninformative operation
hash. Originating a contract results in a fully typed operation hash,
which may be turned into a typed constract handle by the $\CONTRACT$
query. We can ask for the status of any operation hash, for the
storage of a contract handle (if $\TYPE_s = \TNO$, we obtain a useless
value with uninformative type $\TNO$), and for the balance of any address.
% , and $\JTypeValue\STRING\TYPE$ for serialized values as
...
whether the public hash is associated with a smart contract, whether the type of the input parameter matchs with the smart contract's parameter type (chkArg), and whether the fee is
...
the balances of sender and receiver are adjusted (updSucc), the smart contract storage and balance are updated (updConstr), and the time stamp increases.
...
The proofs of these properties refer to all transitions with the detailed specifications of the related functions, such as chkCount and updSucc. Due to page limitations, not all transitions and their associated functions are presented in this paper, so the full proofs will be provided in an upcoming technical report. In this paper, since we only provide the proofs for Proposition 2 at 4 and 7.  

\begin{proof}[4]
 After feeding into a node, the status of the operation is \STATUSPENDING\ according to the transition \TirName{ Node-Inject }. This operation could either be accepted by the blockchain on the condition that the elapsed time is less than 60 ($\TIME - \hat\TIME \le 60$), and then its status is \STATUSINCLUDED\ \TIME \ (the transition \TirName{ Block-Accept }) or it is timeout with the status \STATUSTIMEOUT\ (\TirName{ Block-Timeout}). When an operation is accepted or times out, its status is never changed. Therefore, we have
 
 \begin{itemize}
 \item if the operation remains in \STATUSPENDING, then $\PENDING' (\OPH) = \PENDING (\OPH)$; 
 \item if the operation is timeout, then $\PENDING' (\OPH) = \Angle{\OP, \hat\TIME, \STATUSTIMEOUT}$ according to the transition \TirName{ Block-Timeout}; 
 \item if the operation is accepted, then $\PENDING' (\OPH) = \Angle{\OP, \hat\TIME, \STATUSINCLUDED\ \TIME }$, and $\TIME'=\TIME +1$ because the timestamp is incremented to one when an operation is accepted, according to the transition \TirName{ Block-Accept}.
 \end{itemize}

\end{proof}

\begin{proof}[7]The function $\UPDATECOU(\MANAGERS, \PUK, \BOOLEAN)$ updates the flag of the counter of the account associated with the public key $\PUK$. The specification is as follows:
 \begin{itemize}
   \item[] \UPDATECOU($\PUK \mapsto \langle \BAL, (\NAT, \hat\BOOLEAN) \rangle$, \BOOLEAN) = $\PUK \mapsto \langle \BAL, (\NAT, \BOOLEAN) \rangle $
 \end{itemize}
    
    Function $\UPDATESUCC(\MANAGERS, \PUK, \NTEZ, \MTEZ)$ updates the balance and the counter of the account that associates with the public key $\PUK$. The specification is as follows:
 \begin{itemize}
   \item[]  \UPDATESUCC($\PUK \mapsto \Angle{\BAL, (\NAT, \TRUE)}$, \PUK, \NTEZ, \MTEZ) = $ \PUK \mapsto \Angle{\BAL - \NTEZ - \MTEZ, (\NAT + 1, \FALSE)}$    
 \end{itemize}
    
\noindent Then we have
 \begin{itemize}
   \item[-] $\COU.b=\TRUE$ means that the operation is injected and its status is \STATUSPENDING\ at \TIME according to the transition \TirName{Node-Inject}, and $\COU'.b=\FALSE$ means that the operation is accepted. According to the transition \TirName{Block-Accept} and the specification of the function \UPDATESUCC, the counter of the account is incremented by 1. This means $\COU.n' \in \{\COU.n,  \COU.n+1\}$.
   \item[-] If the operation is still pending or it is time out, the country's number is changed in both cases. This means $\COU.n = \COU'.n$.
   \item[-] If $\COU.n = \COU'.n$, the balance of the account is also changed since the balance  is only changed when the operation is accepted. This means $\BAL = \BAL'$.
 \end{itemize}
\end{proof}
