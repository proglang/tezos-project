...
\end{itemize}
There is an extended version of the paper with further
proofs.\footnote{Available at \url{https://arxiv.org/abs/2108.11867}.}
...
observable behavior. Tezos's smart contract language Michelson and its
...
Pattern matching is the only means to decompose values, cf.\ \PATTERN. The execution model envisions off-chain programs interacting with smart contracts on the blockchain. The programs are defined using expression scripts. The off-chain scripts run on behalf of a single entity. 
...
a contract with parameter type $\TYPE$ and storage type $\TYPEU$. $\TCODE\ \TYPE\ \TYPEU$
...
\item if $\OP = \TRANSFER[\PARAMETER]\NTEZ\PUK\PUH\MTEZ$, then
  ...
  \end{itemize}
\item if $\OP = \ORIGINATE\NTEZ\PUK\CODE\STRING\MTEZ$ and $\STATUS = \STATUSINCLUDED\ \TIME'$, then
  \begin{itemize}
...
sources and the blockchain network. A blockchain oracle is used to provide external data for smart contracts. When the external data is available, an oracle invokes a smart contract with that information. The invocation can conveniently be made through a programmatic interface. There has been extensive research on providing oracle solutions for blockchain.  Adler et al
