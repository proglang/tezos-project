...
\item if $\OP = \TRANSFER[\STRING]\NTEZ\PUK\PUH\MTEZ$, then
  \begin{itemize}
  \item $\PUH\in\DOM (\CONTRACTORS)$
  \item $\STRING : \TYPE_p$
  \item $\Delta (\PUH)  =\TPAIR\ \TYPE_p\ \TYPE_s$. 
  \end{itemize}
\item if $\OP = \ORIGINATE\NTEZ\PUK\CODE\STRING\MTEZ$ and $\STATUS =
  \STATUSINCLUDED\ \TIME'$, then
  \todo{what is the right thing to say here? Where do we obtain the
    $\PUH$? How is it related to the $\OPH$?}
