...
invocation was rejected because of a insufficient fee, we might want
to restart with an increased fee. Or we might decide to wait until the
invocation goes through with the low fee, if it is not that important.

Hence, we would implement a scheme similar to
the bidding strategy: start with a low fee and increase until the
...
The low-level interface that we propose in this paper requires 
