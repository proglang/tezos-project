...
\maketitle
\pagestyle{plain}
\begin{abstract} {\color{blue} Smart contract applications can reach their full potential if software programs can sufficiently interact with smart contracts on the blockchain.}
  Integrating blockchain contracts in software systems requires a
  programmatic interface. We propose a typed interface for this purpose and establish some properties of the combined system. {\color{blue} In detail, we provide an execution model that allows type-safe interaction between programs and the blockchain. Some designed properties are proven by the model.} A
  prototype of the interface is implemented in OCaml for the Tezos blockchain.  {\color{blue} The OCaml API allows user-programs to interact with an auction smart contract on the blockchain in a type-safe manner.}
\end{abstract}
...
{\color{blue} Let us consider how this auction contract could be written in Michelson, the language of smart contracts in Tezos. The state of a smart contract is presented by the data in its storage on the blockchain. The input data provided with a contract invocation is specified as a parameter.
...
where \texttt{unit} is the type whose only value is \texttt{Unit}, \texttt{mutez} is a specific type for manipulating tokens in Tezos, and \texttt{pair} is a pair of values a and b of types (l) and (r) written as (\texttt{Pair a b}).

There are two purposes for calling an auction contract: (1) a bid by a bidder, or (2) a contract closing by the contract owner. Michelson supports multiple entrypoints, distinguished by \texttt{or}. The contract's storage on the blockchain contains two pairs, the first pair containing the flag indicating whether bidding is allowed or the contract is closed and the address of the contract owner, and the second pair containing the current highest bid and the address of the bidder. Michelson is stack-based. It takes a pair of the parameter and the current storage as inputs and returns the updated storage and a list of internal operations. When a bidder invokes the contract to bid, its bid and address are updated in the storage if its bid is higher than the current highest bid, otherwise they are discharged. The closing of the contract is called by the contract owner when it reaches 48 hours }
...
{\color{blue} In contrast, these tasks are able to implement as programs that interact with the blockchain.  We call the function that deploy a contract \texttt{originate-contract}, and the function that invokes a contract \texttt{call-contract}. The \texttt{get-state} function queries data on the blockchain and returns the storage of a contract.
The \texttt{originate-contract} function results the contract's address on the blockchain. The program for a contract owner could be implemented by first deploying the contract, waiting 48 hours, and then calling the contract with the entrypoint \texttt{Unit} to close the contract.
...
Grammar of types
\begin{align*}
  \TYPE, \TYPEU & ::= \TADDR \mid \TOPH \mid \TPUH\ \TYPE\ \TYPEU \mid
                  \TPUK \mid
...
\begin{mathpar}
  ...
  \inferrule{\JTypeCode \CODE{ \TPAIR\ \TYPE_p\ \TYPE_s}}{\JTypeExpr\TEnv\CODE{\TCODE\ \TYPE_p\ \TYPE_s}}
  ...
  \inferrule{
    \JTypeExpr\TEnv{\EXPR_1}\TTEZ \\
    \JTypeExpr\TEnv{\EXPR_2}\TPUK \\
    \JTypeExpr\TEnv{\EXPR_3}\TPUH\ \TYPE_p\ \TYPE_s \\
  ...
  \inferrule{
    \JTypeExpr\TEnv{\EXPR_1}\TTEZ \\
    \JTypeExpr\TEnv{\EXPR_2}\TPUK \\
    \JTypeExpr\TEnv{\EXPR_3}{\TCODE\ (\TPAIR\ \TYPE_p\ \TYPE_s)} \\
    \JTypeExpr\TEnv{\EXPR_4}\TYPE_s \\ % really \TSTRING \\
    \JTypeExpr\TEnv{\EXPR_5}\TTEZ
  }{
    \JTypeExpr\TEnv{\ORIGINATE{\EXPR_1}{\EXPR_2}{\EXPR_3}{\EXPR_4}{\EXPR_5}}{
      \TPUH\ \TYPE_p\ \TYPE_s} 
  }
\end{mathpar}
