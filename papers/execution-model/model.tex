...
\lstinline/left/ or \lstinline/right/ summand. Entrypoint \lstinline/%close/ takes a \lstinline{unit}
parameter and \lstinline/%bid/ takes an amount of tokens of type \lstinline{mutez}.
The \lstinline/storage/ clause declares the state of the contract,
which is a nested pair type indicating whether bidding is allowed (\lstinline{bool}), the address of the
contract owner (to prohibit unauthorized calls to \lstinline/%close/),
the highest bid (of type \lstinline{mutez}), and the bidder's address.
...
the current bid as the new highest bid.
...
let initial_store = ((true, my_address), (1, my_address))
...
library,  \lstinline/originate/ deploys its argument contract along
with the provided funds and the \lstinline/initial_store/, 
\lstinline{auction_script} is the contract from Listing~\ref{lst:auction-contract-header}, and 
\lstinline{call_contract} invokes a contract with a parameter.
...
decompose a set into an element $\mathbf{e}$ and a set  $\mathbf{s}$.
...
\end{align*}
Here, \NTEZ\ and \MTEZ\ are tokens, \PUK\ is the public key of the
...
which is empty for a simple transfer. Furthermore,  \CODE\ is
a script of a smart contract and \INIT\ is the initial value of the
contract's storage. Each operation returns an \emph{operation hash}
\OPH, on which we can query the status of the operation.

The local node also offer several ways to query the current state of
the blockchain.
...
\begin{align*}
  ...
  \\&\mid \QOP\  \EXPR \mid
  \CONTRACT\ \TYPE\ \EXPR \mid \TOSTRING\
...
\end{figure}
All queries and operations are started by closed expressions that run on the
local node. Figure~\ref{fig:syntax-expressions} defines the syntax of
expressions, which describe the usual constructs of call-by-value PCF
with sum, product, list, and option types and exceptions. Moreover, there are
primitive types and constants \CONSTANT\ to support blockchain
...
well as for the queries. Finally, the three constructs 
\CONTRACT\ \TYPE\ \EXPR, \TOSTRING\  \EXPR, and \FROMSTRING\ \TYPE\
\EXPR\ are casts between untyped and typed representations of
...
\begin{figure}[tp]
  \begin{align*}
    \LET\ \OPH &=  \TRANSFER\NTEZ\PUK{\PUK'}\MTEZ \\
         & \MATCH\ \QRY\ \GETSTATUS\OPH\ \WITH \\
         & \mid \STATUSPENDING \to \textrm{not yet included, continue waiting} \\
         & \mid \STATUSINCLUDING (n) \to \textrm{included with $n$
           blocks confirmation}\\
         & \mid \STATUSTIMEOUT \to \textrm{lost operation, try again}
  \end{align*}
  \caption{Pseudocode example for a token transfer}
  \label{fig:pseudocode-example}
\end{figure}
Figure~\ref{fig:pseudocode-example} shows an expression that invokes a
contract at \PUH\ with argument \STRING.

Subtyping in Figure~\ref{fig:subtyping} etablishes that public hashes
and public keys can both stand for addresses. Moreover, a smart
contract with parameter type $\TYPE$ is represented by its public hash
at run time. Subtyping for function types has the usual co-/contravariant definition.
\begin{figure}[tp]
  \begin{mathpar}
    ...
    \inferrule{\TYPEU' \SubType \TYPE' \\ \TYPE \SubType \TYPEU}{
      \TYPE' \to \TYPE \SubType \TYPEU' \to \TYPEU}
  \end{mathpar}
  \caption{Subtyping}
  \label{fig:subtyping}
\end{figure}
