...
Although closing contract and bidding strategy tasks are not able to be implemented in a smart contract, they could be in programs that interact with the blockchain. We call the function that deploy a contract \texttt{originate-contract}, and  it results the contract's address on the blockchain.  The function that invokes a contract is \texttt{call-contract}. The \texttt{get-state} function queries data on the blockchain and returns the storage of a contract. The owner could first deploy the contract, waitg 48 hours, and then call the contract with the entrypoint \texttt{Unit} to close the contract.
