...
\lstinline/%bid/. Entrypoint \lstinline/%close/ takes a \lstinline{unit}
...
Here is an outline of the implementation of the entrypoints. The
...
to the owner, closes the contract, and trusts the owner to deliver the
goods.\footnote{For simplicity we elide safeguarding by a third-party oracle.}
The \lstinline/%bid/ entrypoint rejects and reimburses each bid that is not higher
...
Cl.call_contract(contract, left ());
\end{lstlisting}
We assume that \lstinline/Cl/ is the contract library,  \lstinline/originate/ deploys its argument contract along with
the provided funds and the \lstinline/initial_store/, 
