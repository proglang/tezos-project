...
Grammar of types
\begin{align*}
  \TYPE & ::= \ADDR \mid \OPH \mid \PUH \mid \PUK \mid \CODE\ \TYPE \mid
...
A \emph{pending operation} is a tuple
$\langle  \OP, \OPH, \TIME, \STATUSPENDING\rangle $,
where $\OP$ is an operation, $\OPH$ is the operation hash,
and $\TIME$ is the time when the operation was injected.
Once it is accepted, it changes to an \emph{accepted operation}:
$\langle  \OP, \OPH, \TIME, \STATUSINCLUDED\ \TIME'\rangle $, where
$\TIME'> \TIME$ is the  when the operation was included in the blockchain. 

Properties of blockchain state transitions

\begin{align*}
  P &= \OPH \partialto \OP\times \TIME \times (\STATUSPENDING +
      \STATUSINCLUDED\ \TIME + \STATUSTIMEOUT)
\end{align*}
...
\item for all $\OPH \in \DOM(P)$, if  $\PENDING (\OPH) = \Angle{\OP, \TIME, \STATUS}$, then 
  \begin{itemize}
  \item if $\STATUS = \STATUSTIMEOUT$, then $\PENDING'
    (\OPH) = \PENDING (\OPH)$
  \item if $\STATUS = \STATUSINCLUDED\ \TIME'$,
    then $\PENDING'(\OPH) = \PENDING (\OPH)$
  \item if $\STATUS  = \STATUSPENDING$, then either
    \begin{itemize}
    \item % $\BLOCKCHAIN'.\TIME - \TIME \le 60 $ and
      $\PENDING' (\OPH) = \PENDING (\OPH)$
    \item $\PENDING' (\OPH) =       \Angle{\OP, \TIME, \STATUSINCLUDED\ \BLOCKCHAIN.\TIME}$, 
    \item $\BLOCKCHAIN'.\TIME - \TIME > 60$ and $\PENDING' (\OPH) =
      \Angle{\OP, \TIME, \STATUSTIMEOUT}$, 
    \end{itemize}
  \item $ \DOM (\MANAGERS) \subseteq \DOM (\MANAGERS')$
    (Q: can accounts disappear? Do we want to model that?)
...
\item $\BLOCKCHAIN = [\PENDING, \ACCEPTED, \MANAGERS, \CONTRACTORS,
  ...
  \item
    ...
    c.\PUK$;
  \item if $\PENDING (\OPH) = \Angle{\OP,\TIME, \STATUSINCLUDED\ \TIME'}$,
    then $\TIME' > \TIME$
  \end{itemize}
\end{enumerate}
\end{definition}
