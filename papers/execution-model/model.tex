...
\begin{definition}%[Account]
  ...
  %, \PKH
  \rangle $, where
  % $\ALS$ is the alias of the account,
  $\PAK$ is its private key and $\PUK$ is its public key.
  % and $\PKH$ is its public key hash . 
\end{definition}

% \begin{definition}%[Contract]
%   An \emph{contract} is a tuple $\langle
%   % \ALS,
%   \PUH, \CODE \rangle $, where
%   % $\ALS$ is the alias of the contract, 
%   $\PUH$ is its public hash, and
...
\begin{mathpar}
  \inferrule[Node-Op]
  { % \CHECKACC (\OP.\PUK, \ACCOUNTS)
    \Angle{\PAK,\OP.\PUK} \in \ACCOUNTS
    % \\
    % \OP = \TRANSFER[\STRING]\NTEZ\PUK{\PUK'}\MTEZ
  }{
    [\ACCOUNTS, \OPERATIONS]
    \NodeTrans
  ...
  % \inferrule[Node-Originate]
  % {\CHECKACC (\PUK, \ACCOUNTS) \\
  %   % \CHECKID (\ID, \CONTRACTS) \\
  %   \OP = \ORIGINATE\ID\NTEZ\PUK\CODE\STRING\MTEZ
  % }{
  %   [\ACCOUNTS, \OPERATIONS, \CONTRACTS]
  %   \NodeTrans 
  %   [\ACCOUNTS, \OP :: \OPERATIONS, \CONTRACTS]
  % }
  %
...
%
% \begin{mathpar}
%   \inferrule[Xfer-Propose]{
%     \CHECKACC (\PKH, \ACCOUNTS)
%   }{
% \langle[\ACCOUNTS, \OPERATIONS,
%         \CONTRACTS], [\PENDING, \ACCEPTED, \MANAGERS, \CONTRACTORS,
%         \TIME]\rangle \rightarrow \\
%         \langle [\ACCOUNTS, (\TRANSFER\NTEZ\PKH{\PKH'}\MTEZ) ::
...
\begin{mathpar}
  \inferrule[Block-Timeout]{
    \OP = \TRANSFER[\STRING]\NTEZ\PUK{\PUK'/\PUH}\MTEZ \\
    \TIME'-\TIME \ge 60
  }{
    {[\Angle{\OP,\OPH, \TIME} :: \PENDING, \ACCEPTED, \MANAGERS,
      \CONTRACTORS, \TIME']}
...
\begin{mathpar}
  \inferrule[Block-Originate]{
    ...
    \OP = \ORIGINATE\ID\NTEZ\PUK\CODE\STRING\MTEZ \\
    \OPH = \GENERATEHASH (\ID,\CODE,\STRING,\TIME) 
  }{
    \Angle{[
      \ACCOUNTS, \OP :: \OPERATIONS, \CONTRACTS
...
\begin{mathpar}
  \inferrule[Block-Accept]{
    ...
    \SystemTrans \\
    \langle
      [\ACCOUNTS, \OPERATIONS, \Angle{\ID, \OPH, \CODE} ::\CONTRACTS],
      \\
      [\PENDING, \Angle{\OP, \OPH,\TIME,\TIME'} :: \ACCEPTED, \UPDATESUCC
      (\MANAGERS, \PUK, \NTEZ, \MTEZ), \Angle{\OPH, \GETSTORAGE
      (\CODE,\STRING)} :: \CONTRACTORS, \TIME'+1]
    \rangle
  }
\end{mathpar}

% \begin{equation}{}
% \frac{}{\parbox{4.2in}{\text{$\langle$[\ACCOUNTS, \OPERATIONS,
% \CONTRACTS], [$<$ (originate contract ${id}$ transferring  ${n}$
% from ${pkh}$ running } \\ 
% \text{${code}$ init ${s}$), \TIME $>$ :: \PENDING, \ACCEPTED,
% \MANAGERS, \CONTRACTORS, \TIME' ]$\rangle \rightarrow
% \langle$[\ACCOUNTS, \OPERATIONS,($< id$, } \\ 
% \text{generateHash($id, code, s$, \TIME'), $code >$ :: \CONTRACTS)],
%  [\PENDING, $<$ (originate contract ${id}$ } \\
...
\end{mathpar}
% \begin{equation}{}
% \frac{\text{\TIME' - \TIME $\ge$ 60}}{\parbox{4.2in}{\text{[$<$
% (originate contract ${id}$ transferring  ${n}$ from ${pkh}$ running
% ${code}$ init ${s}$) $>$ } \\ 
% \text{:: \PENDING, \ACCEPTED, \MANAGERS, \CONTRACTORS, \TIME' ]
% $\rightarrow$[\PENDING, \ACCEPTED, updateCou(\MANAGERS, $puk$,
% False), \CONTRACTORS, \TIME']}}}  
% \end{equation}
~\\
~\\
B. \textbf{Transfer}
...
\begin{mathpar}
  \inferrule[Block-Call]{
    ...
    \CHECKARG (\CONTRACTORS, \PUH, \STRING) \\
    \OP =  \TRANSFER[\STRING]\NTEZ\PUK{\PUH}\MTEZ \\
    \OPH = \GENERATEOPH (\PUK,\PUH, \STRING,\NTEZ, \MTEZ, \TIME)
  }{
    \Angle{[\ACCOUNTS, \OP :: \OPERATIONS], 
      [\PENDING, \ACCEPTED, \MANAGERS, \CONTRACTORS, \TIME]      
    }
    \SystemTrans \\
    \Angle{[\ACCOUNTS, \OPERATIONS],
      [\Angle{\OP, \OPH, \TIME} :: \PENDING, \ACCEPTED, \UPDATECOU(
      \MANAGERS, \PUK, \TRUE), \CONTRACTORS, \TIME]
    }
  }
\end{mathpar}
% \begin{equation}{}
...
% \\
% \text{ $\wedge$ checkArg(\CONTRACTORS, $puh,
%   s$)}}{\parbox{4.2in}{\text{$\langle$[\ACCOUNTS, (transfer ${n}$ from
%     ${pkh}$ to  ${puh}$ arg ${s}$ fee ${m}$) :: \OPERATIONS,
%     \CONTRACTS], 
%     [\PENDING, \ACCEPTED, \MANAGERS, \CONTRACTORS, \TIME]
%     $\rangle$}\\
...
\begin{mathpar}
  \inferrule{
    ...
    \UPDATECONSTR (\CONTRACTORS, \PUH, \NTEZ, \STRING),
    \TIME'+1]
  }
\end{mathpar}
% \begin{equation}
