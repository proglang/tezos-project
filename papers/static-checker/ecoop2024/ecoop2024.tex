...
\title{Formal Verification Tool Based on Symbolic Execution for Smart Contract} %TODO Please add
...
Michelson is the low-level smart contract language employed in the Tezos blockchain platform. It is characterized as a stack-based language, featuring high-level data types and primitives and integrates strict static type checking into its stack-based execution mode.

The processing and manipulation of data during the execution of smart contracts are facilitated using a stack. A stack  can be written:
\begin{itemize}
\item[]  \EMPTYSTACK\ for the empty stack.
\item[]  \HEAD\ \STACKCONCAT\ \NTAIL\ for the stack whose first value has type (\HEAD) and queue has stack type (\NTAIL).
\end{itemize}

Each piece of data in Michelson is assigned a specific type, and the language imposes restrictions preventing programmers from introducing custom types. Consider a stack denoted as $\STACK = (\TermOne, \TYF) \STACKCONCAT (\TermTwo, \TYS) \STACKCONCAT \DOT
\STACKCONCAT \EMPTYSTACK$ where elements are types paired with terms. 

A Michelson program is a sequence of instructions executed in a sequential manner. Each instruction takes, as input, the stack produced by the preceding instruction and modifies it for the subsequent one. This sequential flow ensures that each instruction builds upon the state left by its predecessor in the program. Let $\INSTRUCTION = \InstructionOne; \InstructionTwo; \DOT; \InstructionN$ represent a sequence of instructions.

While undergoing symbolic execution,  a symbolic execution state assumes different values based on the constraints of symbolic values. Additionally, branching may occur depending on conditions, and both constraints and branching conditions are recorded as predicates. Let \PREDICATE\ symbolize a predicate expressed in conjunction form, capturing the conditions and constraints encountered during the symbolic execution process.
\begin{definition}
A symbolic execution's state is represented by the tuple $\STATE = [\INSTRUCTION, \STACK, \PREDICATE]$. Here, $\SYSTEM = \{\STATEONE, \STATETWO, \DOT, \STATEN \}$ range over sets of  states.
\end{definition}
The Michelson program takes as input a stack that consists of a single pair whose first element is an input value referred to as the parameter, and second element represents the content of the storage space. The program produces a stack as output, containing a single pair. The first element of this pair is the list of internal operations that the program intends to emit, and the second element is the new contents of the storage space.

Define the initial stack as $S_{init}$   = (\KPAIR\ \VPAR\ \VSTORAGE, \TPAIR\ \TYF\ \TYS) \STACKCONCAT\EMPTYSTACK, where \VPAR\ and \VSTORAGE\ denote terms representing the parameter and the storage, and \TYF\ and \TYS\ are their corresponding types. Similarly, designate the final stack as $S_{final}$   = (\PAIR\ \VOPERATIONLIST\ \VSTORAGE, \TPAIR\ (\TOPERATIONLIST) \TYS) \STACKCONCAT\EMPTYSTACK, where \VOPERATIONLIST\ repesents a operation list.
...
During the execution of symbolic analysis, numberous states are generated. However, certain states may be unreachable from a symbolic initial state. This occurs when the predicate \PREDICATE\ is unsatisfied. Consequently, a system state is disregarded when the predicate \PREDICATE\ is fail.
...
There are various types of instructions in Michelson, categorized based on their functions. These include control structures, stack manipulation, arithmetic operations, boolean operations, cryptographic operations, and operations on data structures. In our model, we classify them into three main groups: basic instructions, branch instructions, and loop instructions. This categorization reflects how these instructions are handled within our specific model.
\subsubsection{One-step Instructions}
A one-step instruction only effects a system state itself, does not produce a new state, and halts the execution after a single operation. The ABS rule specifies the corresponding instruction that applies to the first element of the stack. This element must have the type of int and returns the positive value of its current value, which is of a natural number type. Another example, the COMPARE instruction compares the two first elements of a stack, both of which have a natural number type. It returns a result of either -1, 0, or 1 depending on the comperations of their values.
%ABS
\begin{mathpar}
\inferrule[ABS]
  {
...
Multi-step instructions require the execution of a sub-sequence of instructions before returning to the main execution. The following rule models the instruction EXEC, which executes a function specified as a sequence of instructions denoted as \INSTRUCTIONONE\ in the rule. This instruction first applies the code of the function to the first element of the stack. It then places the result back into the main execution.
\begin{mathpar}
  \inferrule[EXEC]
  {
...
Let  
\LIST\ \STACKCONCAT\ \STACK

 be the stack just before applying the instruction ITER I, where \VariableX\ be the head of the list and \STACK\ be the rest of the stack . For first  loop, let  \STACK\ be f (\VariableX, \STACK). \STACK\ is the stack resulting from applying the function f to the element \VariableX\ and the stack \STACK. In other words, the stack \STACK\ is formed by applying the sequence of the instructions I to the stack \VariableX\ \STACKCONCAT\ \STACK. 

Let the result of applying the instruction \ITER\ I on the stack \LIST\ ::  \STACK\ be the stack S', then Z is resulted from the \FOLD\ function that applies the function f  to each element of the list \LIST\ and initial value is the stack \STACK.
