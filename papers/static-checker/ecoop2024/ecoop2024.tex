...
\title{A Formal Verification Framework for Tezos Smart Contracts Based on Symbolic Execution} %TODO Please add
...
Michelson \cite{michelson}. SCV relies on
...
Our contributions are as follows and supported by the upcoming
artifact submission.
\begin{enumerate}
...
\item Two case studies that verify smart
  contracts deployed on the Tezos blockchain with SCV:
  \begin{itemize}
  \item USDtz, an implementation of the FA~1.2 standard for financial
    applications;
  \item Kolibri, an oracle contract that injects currency exchange
...
perform any visible change. Use of the balance is up to the contract code.
...
Our framework SCV contains a domain-specific language to specify
mathematical properties of smart contracts.
SCV is designed to allow users to write formulas that align with
...
\text{e1}, or \text{Some(v)} if $v$ is the value mapped by the key. Another
function, \text{get\_contract (T, e)}, casts the address \text{e}
to a contract on the Tezos blockchain. It returns \text{None} if
there is no contract on the Tezos blockchain with an
entrypoint of type \text{T} associated with the address \text{e}, or
it returns the contract in the form \text{Some(c)}.
...
The static checker now performs the verification process by employing symbolic execution against the property specification in SCV. It utilizes the results from the previous phases, including symbolic execution, Z3 formulas representing pre and post conditions, and path conditions. 
...
symbolic execution of Michelson programs. We only discuss symbolic
execution for some select instructions. The
