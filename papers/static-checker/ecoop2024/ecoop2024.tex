...
While you can create tokens that behave in any way that you want them to behave, implementing tokens that adhere to established standards is advisable for interoperability and compatibility across different platforms. A Token standard defines a set of rules and an interface that smart contracts must follow to ensure consistency and compatibility with decentralized exchanges, wallets, and other tools within the ecosystem. For example, ERC-20 is a standard used in Ethereum for implementing tokens. 

Tezos blockchain has its own set of standards called Financial Applications (FA). Two popular standards are: (1) FA 1.2, which is designed for fungible tokens, and FA 2, which is more versatile, accommodating both fungible and non-fungible tokens (NFTs). The detail of FA 1.2 is published in \ref{}. In this section, we verify USDtz \ref{}, which is an implementation of the FA 1.2 standard . Our goal is to confirm that the implementation aligns with the standard, ensuring its correctness.  USDtz is written to the FA token standard and The USDtz smart-contract's initial deployment has been under the FA 1.2 standard.


FA 1.2  includes functions such as transfer (transacting between users), approve (allowing one user to send tokens from another user's address), allowance (checking the amount of tokens available for a specific transaction), user balance (checking the balance of a user), and current supply (checking the total supply of tokens across all addresses). A contract which implements approvable ledger must have the following entrypoints:

\begin{lstlisting}[float,captionpos=b,caption={FA 1.2 interface},label={lst:fail-condition-auction-contract-close},numbers=left]
