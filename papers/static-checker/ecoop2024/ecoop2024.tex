...
addresses). The type system in simple in the sense that it does not
support quantification over types.

Listing~\ref {lst:simple-program-add} contains a simple example of a Michelson program.
\begin{lstlisting}[float=tp,captionpos=b,caption={A simple Michelson program},label={lst:simple-program-add},language=michelson]
parameter int;
storage   int;
code  { 
(*               pair int int :: [] *)
...
\end{lstlisting}
\atha{Explain the example program}
\atha{Explain some important instructions for reference later on:
  AMOUNT, BALANCE, FAILWITH, \dots}
\atha{Introduce entrypoints}
