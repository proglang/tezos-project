...
mcontract auction = spec 
  ...
  entrypoint %bid
    ...
    post-condition := auction_open
       ...
       && post_Balance = Amount 
       && transfer_token (Unit, Balance - Amount, c) ;
\end{lstlisting}
...
& \Mid \text{transfer\_token} (\textbf{e}, \textbf{e}, \textbf{e}) 
 ...
 \Mid \text{check\_mem} (\textbf{e}, \textbf{e})  \\
& \Mid \text{get\_contract} (\textbf{e}, \textbf{e})  
 \Mid \text{contain} (\textbf{e}, \textbf{e})  
 \Mid \text{get\_substring} (\textbf{e}, \textbf{e}, \textbf{e})  
 \Mid \text{check\_sig} (\textbf{e}, \textbf{e}, \textbf{e})  \\
& \Mid \text{abs} (\textbf{e})  
...
and output fields, the input and output terms are specified. 
\atha{What terms are supported? terms are not explained!}
Since
...
\pt{I disagree (though it would be a useful feature). To this end, there should be terms to cover stacks
  with more than one element. As it stands, the input term can only
speak about the top of the stack and the output term only about the
...
The input and output terms represent Michelson terms, which can be
either concrete values or symbolic values, including variables. When a variable appears for the first time, it needs to be accompanied by its
...
our case, Z3). The specification language is constituted of predicates.  The syntax terms and  predicates are defined in Figure~\ref{fig:syntax-terms-predicates}. Formulas that are used in assertions or
...
Let us consider the contract \texttt{Sub}  in Listing
...
\lstinline/z/ is greater than or equal to the substract of \lstinline/x/ and \lstinline/y/, which is greater or equal to zero. The symbol \lstinline/&&/ stands for logical conjunction.
...
entrypoints.  There are two options of the syntax as shown in Listing~\ref {lst:multiple-entrypoint-specification-1} and Listing~\ref {lst:multiple-entrypoint-specification-2},
...
list. Given an output term $ \tau$, the checker scans all the final
...
\lstinline|8|. To check this,  the output term may be specified as
a concrete value.
...
2. auction_open && (Amount <= Balance - Amount):
   'Not higher than the current highest bid'
3. auction_open && not (Amount <= Balance - Amount) && 
...
$\Pi$, $\neg$ ($\bigwedge \Phi_{i}$ $\land$ $\Theta$ $\rightarrow \bigwedge \Psi_{j}$) is
...
= \lstinline/x/ - \lstinline/y/) $\rightarrow$ (\lstinline/z/ = \lstinline/x/ - \lstinline/y/) $\land$ (\lstinline/z/ >= 0)) is
unsatisfiable.
