...
(*  printf "big_map_to_self.tz: %s\n" (run_file "./test_files/contracts_alpha/opcodes/big_map_to_self.tz"
  ...
  [%expect {| big_map_to_self.tz: (Pair {} ) |}];*) (*TODO: try-michelson aktzeptiert Eingabe nicht. Zwei unterschiedliche Eingaben wären evtl gut*)
(*  printf "bls12_381_fr_push_bytes_not_padded.tz: %s\n" (run_file "./test_files/contracts_alpha/opcodes/bls12_381_fr_push_bytes_not_padded.tz"
  ...
  [%expect {| chain_id_store.tz: (Pair {} (Some NetXynUjJNZm7wi)) |}]; (*TODO result should change when serialization is properbly implemented *)
