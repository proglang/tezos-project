...
let parse (c : in_channel) : AbsTest.prog =
    ParTest.pProg LexTest.token (Lexing.from_channel c)
;;

let showTree (t : AbsTest.prog) : string =
    "[Abstract syntax]\n\n"^
    (fun x -> ShowTest.show (ShowTest.showProg x)) t^ "\n\n"^
