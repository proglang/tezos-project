...
and prtData (i:int) (e : AbsMichelson.data) : doc = match e with
  ...
  |    AbsMichelson.DPair pair -> prPrec i 0 (concatD [render "Pair" ; prtPair 0 pair])
  ...
  |    AbsMichelson.DMap maps -> prPrec i 0 (concatD [render "{" ; prtMapListBNFC 0 maps ; render "}"])
...
and prtPair (i:int) (e : AbsMichelson.pair) : doc = match e with
       AbsMichelson.DPairSeq1 (data1, data2) -> prPrec i 0 (concatD [prtData 0 data1 ; prtData 0 data2])
  |    AbsMichelson.DPairSeq2 (data, datas) -> prPrec i 0 (concatD [prtData 0 data ; prtDataListBNFC 0 datas])

and prtPairListBNFC i es : doc = match (i, es) with
    (_,[]) -> (concatD [])
  | (_,x::xs) -> (concatD [prtPair 0 x ; prtPairListBNFC 0 xs])
and prtMap (i:int) (e : AbsMichelson.map) : doc = match e with
       AbsMichelson.DMapSeq (data1, data2) -> prPrec i 0 (concatD [render "Elt" ; prtData 0 data1 ; prtData 0 data2 ; render ";"])

and prtMapListBNFC i es : doc = match (i, es) with
    (_,[]) -> (concatD [])
  | (_,[x]) -> (concatD [prtMap 0 x])
  | (_,x::xs) -> (concatD [prtMap 0 x ; render ";" ; prtMapListBNFC 0 xs])
and prtInte (i:int) (e : AbsMichelson.inte) : doc = match e with
       AbsMichelson.DIntPos integer -> prPrec i 0 (concatD [prtInt 0 integer])
...
and prtInstr (i:int) (e : AbsMichelson.instr) : doc = match e with
  ...
  |    AbsMichelson.CREATE_CONTRACT instrs -> prPrec i 0 (concatD [render "CREATE_CONTRACT" ; render "{" ; prtInstrListBNFC 0 instrs ; render "}"])
