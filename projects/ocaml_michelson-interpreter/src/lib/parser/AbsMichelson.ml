...
let d_ASSERT_SOME annots = IF_NONE (d_FAIL :: [], RENAME :: [])
let d_ASSERT_LEFT annots = IF_LEFT (RENAME :: [], d_FAIL :: [])
let d_ASSERT_RIGHT annots = IF_LEFT (d_FAIL :: [], RENAME :: [])
let d_IF_SOME (bt, bf) = IF_NONE (bf, bt)
let d_IF_RIGHT (bt, bf) = IF_LEFT (bf, bt)
let m_MAP_CAR (annots, code) = BLOCK (DUP :: CDR :: (DIP (CAR :: (BLOCK code) :: [])) :: SWAP :: PAIR :: [])
let m_MAP_CDR (annots, code) = BLOCK (DUP :: CDR :: (BLOCK code) :: SWAP :: CAR :: PAIR :: [])
