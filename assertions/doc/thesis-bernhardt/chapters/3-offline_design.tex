...
A file containing assertions (henceforth referred to as ``assertion contract'') contains at least one assertion for some function or entrypoint of the original contract (henceforth referred to as ``parent contract''). An assertion begins with a signature consisting of an optional tag denoted by \%, followed by a parameter pattern. The tag can be used for documentation purposes, but may sometimes be needed to indicate which function of the parent contract the assertion should be assigned to by mirroring the entrypoint name. Parameter patterns are similar to patterns in OCaml --- they are templates that match shapes of data structures and binds identifiers to the components of them. The pattern given in the signature must match the parameter type of the entrypoint it refers to. The body is an (optional) nesting of quantifiers and conditions around exactly one assertion. Conditionals can be used to restrict the quantification domains or for some other constraints. For completeness, the existential quantifier is already included in the grammar, however the current version of the pipeline will reject any assertions containing it. \\
...
\end{equation}
The respective assertion that checks this property for the parent contract, given in \lstref{lst:sorted}, is expressed in \lstref{lst:sorted_assertion}.
...
\end{figure}
OCaml was chosen as a programming language to implement the pipeline, given that it's also used to develop the Tezos blockchain. This avoids context switches within the Tezos eco-system and allows to build on top of the Tezos libraries (cf. \secref{sec:tezos}).
