...
\vspace{-30pt}
 \begin{center}
  ...
  \end{center}
  \caption{Abstract syntax tree of the assertion given in \lstref{lst:sorted_assertion}}
...
Pursuing the example of the assertion for sorted lists, the negated version of the AST given in \figref{fig:ast_example} is shown in \figref{fig:ast_example_neg}.
% point out negated expressions
\begin{figure}
\subfloat[Negated AST\label{fig:ast_example_neg}]{%
  \includegraphics[width=0.45\textwidth]{figures/3-offline/ast_example_neg}
}
\hfill
\subfloat[Fully transformed AST\label{fig:ast_example_transf}]{%
  \includegraphics[width=0.4\textwidth]{figures/3-offline/ast_example_transf}
}
...
% Restricting the ranges of the random generators is important in order to avoid wasting resources through testing values out of the relevant (or legal) scope.
