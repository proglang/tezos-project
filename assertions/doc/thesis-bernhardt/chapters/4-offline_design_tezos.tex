...
Tezos specific costs analysis of chosen orchestration strategy -> PROBLEM: high n might reach operation gas limit?

\section{Open questions}

\subsection{Representing counterexamples}
\draft{}
- the random number somehow has to be extracted from the contract execution, s.t. other validators can verify the counterexample
- failwith exposes top element of the stack in error message -> can be used to "return" the random values? Question for protocol design how to process it further
- if this is not possible, Michelson may have to be extended with a special failure message that can be handled differently
...
- open question how the validators can check the counterexample -> send piece of code with random values as parameter?
- incentives \& penalties part of the block chain protocol