...
- fixes the problem of the monolithic approach, where calls to default cause problems -> mirrored interface can simply pass the parameter on as is to the parent and the extracted raw value to the assertion
- parent code doesn't have to be touched -> works for originated contracts
- flexibility -> case contracts independent of the rest; replace manager (with or without case contracts), parent with manager
- assertions can be bypassed in any case -> big security issue

\begin{itemize}
\item The address of the assertion contract needs to be provided in some way
	\begin{itemize}
	\item [a)] Parent storage contains the address of its assertion contract
		\begin{itemize}
		\item Parent contract needs to be modified (storage declaration, every storage access \& return value)
		\item Assertion contract must be originated first, so that address can be passed into initial storage of parent
		\item Baker needs to retrieve address from contract storage (no obvious standard way to do this - storages can look differently)\\
		 \verb|->| BigMap?
		\end{itemize}
	\item [b)] Address of the assertion contract is a parameter of the transaction
		\begin{itemize}
		\item Address must be known to the caller
		\item Decouples parent and assertion contract %\sout{(if calling assertion from parent is not necessary)}
		\item Safety issue: can be passed a contract with correct type that never returns a counterexample
		\item Parent contract doesn't need to be touched
		\end{itemize}
	\end{itemize}
\end{itemize}
