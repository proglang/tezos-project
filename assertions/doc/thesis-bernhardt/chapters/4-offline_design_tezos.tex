...
As the algorithm generates the core code for each entrypoint only, another code sequence that prepares the stack similar to \lstref{lst:contract_templ} must be prepended.



% generate_loop template; i is hardwired;  (works for contract w 1 entrypoint), last op first
