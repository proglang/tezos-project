...
% the chapter identifies/describes necessary amendments to Michelson and the VM -> part of protocol design, but also concerns the toolchain -> secref
% furthermore, a scheme for the orchestration of assertion and parent code is required. the code somehow needs to collude -> explores some options; discusses advantages and disadvantages. picks a winner for the proof-of-concept impl.
% for background/ as a basis for this chapter, section 1 gives an introduction to Tezos and Michelson
...
\begin{figure}[!htb]
\subfloat[Appending assertions as separate entrypoints]{\includegraphics[width=0.5\textwidth]{figures/5-offline_tezos/pipeline_output_mono_ep_basic.jpg}}
\quad
\subfloat[Inserting the assertion code into the original entrypoint]{\includegraphics[width=0.5\textwidth]{figures/5-offline_tezos/pipeline_output_mono_basic.jpg}}
